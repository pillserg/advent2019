...
def find_taxicab_distance(tup1, tup2):
    return abs(tup1[0] + tup2[0]) + abs(tup1[1] + tup2[1])


def find_closest_crossection(d1, d2):
    crossections = find_crossections(parse_wire_input(d1), parse_wire_input(d2))
    curr_shortest = None
    for t in crossections:
        d = find_taxicab_distance(t, (0, 0))
        if not curr_shortest:
            curr_shortest = d
        if d < curr_shortest:
            curr_shortest = d
